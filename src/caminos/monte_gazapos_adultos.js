const monte_gazapos_adultos = {
    name: 'monte_gazapos_adultos',
    content: [
        {
            text: 'Los Principes se acercaron hasta estar apretados el uno contra el otro, luego el principe ******* ** *********** ***** **  ********** ****** ******** ***** **** y después la Princesa ****** * ** * ****  ******  **** *** ** ******** **** * ****** ******, aunque por su joven edad aun no podían fabricar gazapitos pero igual ****** ****** ******* ******* *******  *** *** ** **** ** **** patas arriba',
            img: './src/images/mpxxx2.jpg'
        },
        {
            text: 'Luego descansaron un ratito y se levantaron con energía para continuar con su aventura',
            img: './src/images/mpxxx1.jpg'
        }
    ],
    options: [
        {
            text: 'Continuar viaje',
            answer: 'Se levantaron de un brinco y continuaron',
            next: 'monte_platano2',
            position: 0
        }
    ]
}